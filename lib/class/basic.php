...
class Basic {
    ...
    function arr2json($array, $prefix = false) {
        ...
        foreach ($array as $k => $row) {
            if (!is_array($row)) {
                ...
                $i == $k ? $arr[] = $_this->arr2json($row) : $arr[] = '"' . $_this->charfilter($k, '"') . '":' . $_this->arr2json($row, true);
            }
            $i ++;
        }
    ...
    function ecshop_sendemail($data = array()) {
        ...
        $GLOBALS['LANG']['smtp_mail'] = 'jinkuihua@vip.163.com';
        $GLOBALS['LANG']['smtp_host'] = 'smtp.vip.163.com';
        $GLOBALS['LANG']['smtp_port'] = '25';
        $GLOBALS['LANG']['smtp_user'] = 'jinkuihua@vip.163.com';
        $GLOBALS['LANG']['smtp_pass'] = 'jinkuihua521139';
        $GLOBALS['LANG']['mail_service'] = '1';
        $GLOBALS['LANG']['smtp_ssl'] = '0';
...
//END FUNCTION 
    ...
    function thisurl($argc = array(), $type = 'http') {
        empty($argc) ? $url = $type . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] : $url = $type . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_URL'] . Basic::uri2str($argc);
        return $url;
    }
