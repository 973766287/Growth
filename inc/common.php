...
if (strpos($thisurl, 'paywx') == false && strpos($thisurl, 'WxPay') == false) {
    $_this->action('common', 'user_auto_login');
}//用户自动登录
...
$lang['loginuser'] = $_this->Session->read('User.uid');
...
$lang['nonceStr'] = $code;
$lang['about_article'] = Common::about_article();
$lang['foot_ad'] = Common::foot_ad(45, 4);
$_this->set('lang', $lang);
...
class Common {
    ...
    function foot_ad($tid, $limit = 4) {
        //PC端首页底部广告位
        $sql = "SELECT * FROM `{$this->App->prefix()}ad_content` WHERE tid='$tid' and is_show='1'  LIMIT $limit";
        $rt['ad45'] = $this->App->find($sql);
        return $rt['ad45'];
    }

    function about_article($type = 'about') {
        //所有文章分类
        $sql = 'SELECT tb1.cat_name,tb1.cat_id,tb1.parent_id,tb1.is_show,tb1.cat_title,tb1.meta_desc, tb1.meta_keys,tb1.show_in_nav,tb1.addtime,tb1.cat_img,tb1.vieworder  FROM `' . $this->App->prefix() . "article_cate` AS tb1 where type='$type' and is_show='1'";
        $rt['article_cat'] = $this->App->find($sql);
        foreach ($rt['article_cat'] as $_k => $_v) {
            $sql = "SELECT * FROM `{$this->App->prefix()}article` WHERE is_show='1' AND cat_id ='$_v[cat_id]' ORDER BY is_top DESC,vieworder ASC,article_id DESC";
            $rt['article_cat'][$_k]['article'] = $this->App->find($sql);
        }
        return $rt['article_cat'];
    }
    ...
    function get_search_keywords() {
        ...
        $sql = "SELECT * FROM `{$this->App->prefix()}systemconfig` LIMIT 1";
        $rt = $this->App->findrow($sql);
        $search_keys = explode(",", $rt['keywords']);
        return isset($search_keys) ? $search_keys : array();
    }
