		...
		$.ajax({
		   ...
		   success: function(data){ 
				...
				if(data.error==""){
					...
					JqueryDialog.Open('金葵花网登录系统',return_login_string('comment',goods_id),300,50);
...
function ajax_check_comment(gid){
	...
	$.post(SITE_URL+"user.php",{action:"getuid"},function(data){ 
		...
		if(uid>0){
			JqueryDialog.Open('金葵花网评论系统',return_comment_string(gid),450,300);
		}else{
			JqueryDialog.Open('金葵花网登录系统',return_login_string('comment',gid),300,50);
		}
	});
}
...
//分类商品
function get_categoods_page_list(page,cid,bid,price,order,sorts,limit,attr,supplier_id){
