...
function checkloginvar(){
		...
		if(user=="" || typeof(user)=='undefined'){
			$('.uname_mes').html("<font color='red'>用户账号不能为空！</font>");
			return false;
		}
		clearloginmes();
...
function submit_login_data(){
		...
		if(name == "" || pas == ""){ alert("请输入完整的用户名及密码！!"); return false; }
		createwindow();
		$('.login_mes').html('登陆...');
		$.post(SITE_URL+'user.php',{action:'login',username:name,password:pas,vifcode:vcode,issave:issave,isauto:isauto},function(data){
			removewindow();
			if(data != ""){
				//meswindow('<br /><p>'+data+'</p>','金葵花商城登录系统提醒你',300,80);
				//JqueryDialog.Open('Stybags.com Sign-on System','<br /><p>'+data+'</p>',300,50);
				$('.login_mes').html(data);
			}else{
				$('.login_mes').html('登陆成功');
				//meswindow('<br /><p>登录成功！3秒后进入前一个页面！</p>','金葵花商城登录系统提醒你',300,80);
				JqueryDialog.Open('金葵花网登陆系统','<br /><p style="line-height:26px">登陆成功，系统将带你进入前一个页面！</p>',300,50);
				if(jumpurl!="" && jumpurl!=null && typeof(jumpurl)!='undefined'){
						setTimeout("jsjump('"+jumpurl+"')",3000);
				}else{
...
/*function submit_login_data2(){
		//if(checkloginvar()==false) return false;
		
		name = $('.content input[name="userid"]').val();
		...
		if(typeof(issave)=='undefined' || issave=="" || issave==null) issave = 0; //保存用户登录
		if(typeof(isauto)=='undefined' || isauto=="" || isauto==null) isauto = 0;
		if(name == "" || pas == "" || vcode == ""){ alert("请输入完整信息！"); return false; }
		createwindow();
		$.post(SITE_URL+'user.php',{action:'login',userid:name,password:pas,vifcode:vcode,issave:issave,isauto:isauto},function(data){
			removewindow();
			if(data != ""){
				meswindow('<br /><p>'+data+'</p>','金葵花商城登录系统提醒你',300,80);
			}else{
	   ...
	   if (form)
	   {
		...
		$.ajax({
		   ...
		   success: function(data){ 
		   		removewindow();
				if(data.error==0){
					JqueryDialog.Open('注册成功，10秒后进入后台',SITE_URL+'pop/regsuccess.php',400,220,'frame');
					
					setTimeout('location.reload()',10000);
					/*if(jumpurl!="" && jumpurl!=null && typeof(jumpurl)!='undefined'){
						meswindow('<br /><p>你已经注册成功！三秒后将跳转前一个页面！</p>','金葵花商城购物车系统提醒你',300,80);		
						setTimeout("jsjump('"+jumpurl+"')",3000);
					}else{
						location.href=SITE_URL+'user.php?act=regsuccess'; //注册成功
					}*/
				}else{
					JqueryDialog.Open('金葵花商城系统提醒你','<br /><p>'+data.message+'</p>',300,80);
				}
		   },
...
//修改用户信息
function update_user_info(types){
	   ...
	   fromAttr.types = types;
	   
	   createwindow();
	   $.ajax({
		   type: "POST",
		   url: SITE_URL+"user.php?action=updateinfo",
		   data: "fromAttr=" + $.toJSON(fromAttr),
		   dataType: "json",
		   success: function(data){
			   ...
			   removewindow();
		   		...
		   		if(data.error==3){
					if(types=='10'){
						...
						window.location.href=SITE_URL+'user.php?act=login';
					}
				}else if(data.error==0){
                                 
					JqueryDialog.Open('系统提醒你','<br />修改成功！',250,50);
                                        window.location.href=SITE_URL+'user.php';
...
//修改用户密码
function update_user_pass(types){
	   var fromAttr        = new Object();  //
	   var form      = document.forms['USERINFO']; //
	   if(form){
			fromAttr = getFromAttributes(form);
...
function get_order_page_list(pag,statu){
	...
	$.post(SITE_URL+'user.php',{action:'getorderlist',time:times,status:statu,keyword:kkk,page:pag},function(data){
		removewindow();
...
//配送店客户订单查询
function get_store_page_list(pag,statu){
	createwindow();
	if(pag=="" || typeof(pag)==null ) pag = 1;
	times = $('select[name="dt"]').val();
	//statu = $('select[name="status"]').val();
	...
	if(typeof(statu)=='undefined'){ alert("请确认Html的元素是否存在！"); return false;}
	$.post(SITE_URL+'store.php',{action:'getorderlist',time:times,status:statu,keyword:kkk,page:pag},function(data){
		removewindow();
		if(data !="" && typeof(data) != 'undefined'){
			$('.AJAXORDERLIST').html(data);
		}
...
/*function get_store_page_list(pag,statu,times,kkk){
	createwindow();
	if(pag=="" || typeof(pag)==null ) pag = 1;
	//times = $('select[name="dt"]').val();
...
function ressinfoop(id,type,obj){ 
		...
		$.post(SITE_URL+'user.php',{action:'ressinfoop',id:id,type:type},function(data){
			if(type=='showupdate'){																	  
				if(data !="" && typeof(data) != 'undefined'){
					...
					JqueryDialog.Open('金葵花商城提醒你',data,400,300);
				}
			}else if(type=='delete'){
				...
				$(obj).attr('src',SITE_URL+'theme/images/quxiaodefaultress.png');
				...
				$(obj).attr('src',SITE_URL+'theme/images/setdefaultress.png');
				ons = $(obj).attr('onclick');
	...
	if(dd==true){
		  ...
		  attrbul.id = id;
		  attrbul.type = type;
		  createwindow();//alert($.toJSON(attrbul));return false;
		  $.ajax({
			   type: "POST",
			   ...
			   success: function(data){
					...
					if(data.error==0){
						...
						JqueryDialog.Open('金葵花商城提醒你','<br /><img src="'+SITE_URL+'theme/images/error_icon.png" align="absmiddle"/>&nbsp;&nbsp;&nbsp;'+data.message,260,60);
					}
...
//入驻协议zzzzzzzzzzzzzzzzzzzzzzzzzz
function apply_one(){
	 
	  var agreement = document.getElementById("input_apply_agreement").checked;
	   
	   createwindow();
	   $.ajax({
		   type: "POST",
		   url: SITE_URL+"user.php?action=apply_one",
		   ...
		   success: function(data){
			   ...
			   removewindow();
				...
				JqueryDialog.Open('系统提醒你','<br />'+data.message,250,50);
                                        ...
                                        window.location.href=SITE_URL+'user.php?act=apply_two';
                                       // $('table .returnmes').html(data.message);
