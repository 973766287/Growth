function ajax_cart_checkout(uid){
	if(uid>0){
		...
		meswindow(return_login_string('checkout',0),'金葵花网登录系统',300,150);	
	}
	return false;
}
  ...
  $.ajax({
	   ...
	   success: function(data){
			 ...
			 if(tt=='cartlist'){ //购物车列表页面
				if(data.error==4){JqueryDialog.Open('金葵花网提醒你','<br />'+data.message,300,50); return false; }
				else if(data.error==5){ //Product attributes exist
					  JqueryDialog.Open('金葵花网提醒你',data.message,300,200);
				}
				...
				$.post(SITE_URL+"ajaxfile/mycart.php",{action:'delcartid',id:0},function(data){
				 ...
				 if(data.error==3){ //Need login
					 JqueryDialog.Open('金葵花网提醒你登陆系统',return_login_string('jifen',goodsid),300,50);
				 ...
				 JqueryDialog.Open('金葵花网提醒你',str,300,50);
				 }
			 }else if(tt=='jifen_cartlist'){ //Redeem Points goods
				 if(data.error==3){ //Need login
					 JqueryDialog.Open('金葵花网登陆系统',return_login_string('jifen',goodsid),300,50);
				 }else if(data.error==5){ //Product attributes exist
					  JqueryDialog.Open('金葵花网提醒你',data.message,300,200);
				 }else{
					if(data.error==2){
						str = '<br />'+data.message+'<br /><p style="width:175px; position:relative"><a href="'+SITE_URL+'mycart.php?type=cartlist" onclick="window.location.href=\''+SITE_URL+'mycart.php?type=cartlist\'" style="display:block; height:25px; line-height:25px; width:80px;background-color:#B40C0C; position:absolute; left:0px; bottom:-30px;">查看购物车</a>&nbsp;<a href="javascript:;" onclick="JqueryDialog.Close();" style="display:block; height:25px; line-height:25px; width:80px;background-color:#B40C0C; position:absolute; right:0px; bottom:-30px;">继续购物</a></p>';
				        JqueryDialog.Open('金葵花网提醒你 Remind you',str,300,50);
					}else{
						createwindow();
						$.post(SITE_URL+"ajaxfile/mycart.php",{action:'delcartid',id:0},function(data){
								$('.MYCART').hide();
				 ...
				 if(data.error==5){ //存在商品属性
					...
					if(data.error==0){
						...
						meswindow('<br /><br />'+data.message,'金葵花网提醒你',300,100);		
					}
				 }
			 }
			 ...
			 if(tt=='cartlist'){ //购物车列表页面
				...
				$.post(SITE_URL+"ajaxfile/mycart.php",{action:'delcartid',id:0},function(data){
				 ...
				 if(data.error==3){ //需要登录
					 JqueryDialog.Open('金葵花网登录系统',return_login_string('jifen',goodsid),300,50);
				 }else if(data.error==5){ //存在商品属性
					  JqueryDialog.Open('金葵花网提醒你',data.message,300,200);
				 }else{
				 ...
				 if(data.error==5){ //存在商品属性
					...
					if(data.error==0){
						...
						var ss = "";
						$.post(SITE_URL+'ajaxfile/popbox.php',{action:'box',boxname:filename,gid:goodsid,type:"cart",num:goods.number},function(data){
							//meswindow(data,'金葵花网购物车系统提醒你',500,180);
							$('.MYCARTTOP').html(data);
						...
						meswindow('<br /><br />'+data.message,'金葵花网购物车系统提醒你',300,100);		
					}
				 }
			 }
	...
	$.post(SITE_URL+'ajaxfile/goods.php',{action:'addtocoll',goods_id:gid},function(data){ 
		...
		*/
		...
		if(data==1){
			...
			JqueryDialog.Open('金葵花商城提醒你',str,300,90);
			 
		}else if(data==2){
			//JqueryDialog.Open('金葵花商城登陆系统',SITE_URL+'pop/login.php?gid='+gid+'&tt=coll',400,230,'frame');
			JqueryDialog.Open('金葵花网登录系统',return_login_string('coll',gid),300,50);
		}else if(data==3){
			...
			str = '<br />该商品已经存在收藏夹中！<br /><p class="opitem"><a href="javascript:;" onclick="location.href=\''+SITE_URL+'user.php?act=mycoll\'" class="collview">立即查看</a>&nbsp;<a href="javascript:;"  onclick="JqueryDialog.Close();" class="collcolse">关闭</a></p>';
			 JqueryDialog.Open('金葵花商城提醒你',str,300,110);
		}else{
	...
	$.post(SITE_URL+'ajaxfile/goods.php',{action:'addtocoll',goods_id:gid},function(data){ 
		...
		*/
		...
		if(data==1){
			...
			meswindow(str,'金葵花网提醒你',300,110);	
			//JqueryDialog.Open('金葵花网提醒你',str,300,40);
			 
		}else if(data==2){
			
			//JqueryDialog.Open('金葵花网登录系统',return_login_string('coll',gid),300,50);
			meswindow(return_login_string('coll',gid),'金葵花网登录系统',300,150);	
		}else if(data==3){
...
/*############################################*/
//评论处理区
function submit_comment(goods_id,supplier_id){
		...
		$.ajax({
		   ...
		   success: function(data){ 
				...
				if(data.error==""){
					...
					JqueryDialog.Open('金葵花网登录系统',return_login_string('comment',goods_id,supplier_id),300,50);
...
function submit_message(goods_id){
		...
		if(formObj){
			...
			JqueryDialog.Open('金葵花网提醒你','不存在留言表单对象！',300,50);
			return false;
		}
		...
		$.ajax({
		   ...
		   success: function(data){
				if(data.error==0){
					JqueryDialog.Open('金葵花网提醒你','提问成功，我们会很快回复你提出的问题！',300,50);
					$('.GOODSMESSAGE').html(data.message);	
				}else if(data.error==2){
					$('.message_mes').html(data.message);
				}else{
					JqueryDialog.Open('金葵花网提醒你','<br />'+data.message,300,50);
				}
		   } //end sucdess
		}); //end ajax
} // end function
...
function ajax_check_comment(gid,sid){
	var uid = 0;
	$.post(SITE_URL+"user.php",{action:"getuid"},function(data){ 
		...
		if(uid>0){
			JqueryDialog.Open('金葵花网评论系统',return_comment_string(gid,sid),450,300);
		}else{
			JqueryDialog.Open('金葵花网登录系统',return_login_string('comment',gid,sid),300,50);
		}
	});
}
...
function ajax_check_message(gid,gname){
	...
	$.post(SITE_URL+"user.php",{action:"getuid"},function(data){ 
		...
		if(uid>0){
			JqueryDialog.Open('金葵花网商品提问系统',return_message_string(gid),405,270);
		}else{
			JqueryDialog.Open('金葵花网登录系统',return_login_string('message',gid),300,50);
		}
	});
}
//购买历史
...
//分类商品
function get_categoods_page_lists(page,supplier_id,cid,bid,price,order,sorts,limit,attr){
	...
	if(attr==""||typeof(attr)=='undefined') attr="";
	arr.attr = attr;
	if(supplier_id==""||typeof(supplier_id)=='undefined') supplier_id=0;
	...
	$.ajax({
		   type: "POST",
		   url: SITE_URL+"ajaxfile/ajax.php",
		   data: "type=ajax_getcategoodslist&func=categorys&goodswhere=" + $.toJSON(arr),
		   dataType: "json",
		   success: function(data){
