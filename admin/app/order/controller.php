...
class OrderController extends Controller{
	...
	function order_list(){
		//获取支付方式
			...
			$payres = $this->App->find($sql);
		//	print_r($payres);
			$this->set('payres',$payres);
			...
			//支付方式查询  2016-09-10 15:44
			  if(isset($_GET['pay_id'])&&!empty($_GET['pay_id'])&&$_GET['pay_id'] > 0){
                    $comd[] = "i.pay_id  = ".trim($_GET['pay_id']);
                    	}
                    	
                    	//商户号查询 2016-09-10 15:44
                    if(isset($_GET['user_id'])&&!empty($_GET['user_id'])){
                    $comd[] = "i.user_id  = ".trim($_GET['user_id']);
                    	}
                    
                    	//查询法人姓名 2016-09-10 17:20
			  if(isset($_GET['uname'])&&!empty($_GET['uname'])){
                    $comd[] = "b.uname LIKE '%".trim($_GET['uname'])."%'";
                    	}
                    	
                    	                    	//查询法人手机 2016-09-10 17:20
			  if(isset($_GET['mobile'])&&!empty($_GET['mobile'])){
                    $comd[] = "b.mobile LIKE '%".trim($_GET['mobile'])."%'";
                    	}
                    	
                    	                    	//查询法人身份证 2016-09-10 17:20
			  if(isset($_GET['idcard'])&&!empty($_GET['idcard'])){
                    $comd[] = "b.idcard LIKE '%".trim($_GET['idcard'])."%'";
                    	}
                    	
                    	
            if(isset($_GET['order_sn'])&&!empty($_GET['order_sn']))
                    $comd[] = "i.order_sn LIKE '%".trim($_GET['order_sn'])."%'";
			/***********  look添加 日期查询 开始  ********************************/	
			if(isset($_GET['add_time1'])&&!empty($_GET['add_time1']) && empty($_GET['add_time2'])){
					$t = strtotime($_GET['add_time1'])+24*60*60 ;
                    $comd[] = "i.add_time >= ". strtotime($_GET['add_time1']) ."&&i.add_time < " .$t;
					}
			if(isset($_GET['add_time2'])&&!empty($_GET['add_time2']) && empty($_GET['add_time1'])){
                    $comd[] = "i.add_time <= ". strtotime($_GET['add_time2']);
					}
            ...
            if(isset($_GET['consignee'])&&!empty($_GET['consignee']))
                    $comd[] = "i.consignee LIKE '%".trim($_GET['consignee'])."%'";
            ...
            $list = 12;
            $start = ($page-1)*$list;
            $sql = "SELECT COUNT(order_id) FROM `{$this->App->prefix()}goods_order_info`  AS i ";
            $sql .= "LEFT JOIN `{$this->App->prefix()}user_bank` AS b ON i.user_id = b.uid  "; //加入查询  姓名  手机号  身份证号 2016-09-10 17:16
            $sql .= " $w";
            ...
            $sql = "SELECT i.*, SUM( i.order_amount + i.shipping_fee ) AS tprice FROM `{$this->App->prefix()}goods_order_info` AS i ";
             $sql .= "LEFT JOIN `{$this->App->prefix()}user_bank` AS b ON i.user_id = b.uid  "; //加入查询  姓名  手机号  身份证号 2016-09-10 17:16


      $chaxun= $sql ." $w GROUP BY order_id $orderby ";
        if($_POST[sss]!="sss") {
    $sql .= " $w GROUP BY order_id $orderby LIMIT $start,$list";
...
//		    echo $sql;
            ...
            if(!empty($rt)){
                foreach($rt as $row){
                    ...
                    $user_rank = $this->App->findvar("SELECT user_rank FROM `{$this->App->prefix()}user` WHERE user_id = '$uid' LIMIT 1");
					
					if($row['feilv']){
			 $feilv2 = ($row['feilv']/10000);
			}
			else{
                    $feilv = $this->App->findvar("SELECT discount FROM `{$this->App->prefix()}user_level` WHERE lid = '$user_rank' LIMIT 1");
                    $feilv2 = $feilv/10000;
			}
                    $orderlist[] = array(
						...
						'uname'=>$this->App->findvar("SELECT uname FROM `{$this->App->prefix()}user_bank` WHERE uid = '$uid' LIMIT 1"),//真实姓名  user_bank
			...
			if($su == 'yes'){
			$shoppinglist = $this->App->find("SELECT * FROM `{$this->App->prefix()}supplier_shipping_name` where supplier_id=".$orderlist['supplier_id']);
			...
			switch($ty){
				...
				case 'back': //退货单列表
					...
					case 'supplier':
						$this->template('goods_order_supplier_list');
						break;
				default:
...
//            }
	...
	function ajax_get_status_button($var=0){
		...
		function paymentlist(){
			...
			$rt = $this->App->find("SELECT * FROM `{$this->App->prefix()}payment` where is_online=0");
			$this->set('rt',$rt);
		...
		function banklist(){
			//删除
				$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
				if($id>0){
				$this->App->delete('bank','id',$id);
				}
				
			$rt = $this->App->find("SELECT * FROM `{$this->App->prefix()}bank`");
			$this->set('rt',$rt);
			 $this->template('banklist');
		}
		...
		function paymentinfo($id=0){
			...
			if(!empty($_POST)){
				...
				$data['pay_config'] = serialize(array('pay_no'=>(isset($_POST['pay_no'])?$_POST['pay_no']:""),'pay_code'=>(isset($_POST['pay_code'])?$_POST['pay_code']:""),'pay_idt'=>(isset($_POST['pay_idt'])?$_POST['pay_idt']:""),'pay_address'=>(isset($_POST['pay_address'])?$_POST['pay_address']:"")));
			...
			if($id>0){
				if(isset($data)&&!empty($data)){
					...
					if($this->App->update('payment',$data,'pay_id',$id)){
						...
						$this->action('common','showdiv',"payment.php?type=");
					}else{
				...
				$rt['pay_address'] = $pay_config['pay_address'];
		...
		function bankinfo($id=0){
			...
			if($id>0){
				if(isset($data)&&!empty($data)){
					...
					if($this->App->update('bank',$data,'id',$id)){
						$this->action('system','add_admin_log','更新银行：'.$data['name']);
						$this->action('common','showdiv',"payment.php?type=banklist");
					}else{
						$this->jump('payment.php?type=bankinfo&id='.$id,0,'更新失败！'); exit;
					}
				}
				$rt = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}bank` WHERE id='$id' LIMIT 1");
				$pay_config = unserialize($rt['pay_config']);
				$rt['pay_no'] = $pay_config['pay_no'];
				$rt['pay_code'] = $pay_config['pay_code'];
				$rt['pay_idt'] = $pay_config['pay_idt'];
				...
				if(!empty($data)){
					...
					if($this->App->insert('bank',$data)){
						$this->action('system','add_admin_log','添加银行：'.$data['name']);
						$this->action('common','showdiv',"payment.php?type=banklist");
					}else{
						$this->jump('payment.php?type=bankinfo',0,'添加失败！'); exit;
					}
				}
				$type = 'add';
			}
...
//                    $w .= " and pay_time >= ". strtotime($arr['add_time1']) ." and pay_time < " .$t;
			...
			if(isset($arr['pay_id'])&& $arr['pay_id'] > 0){
				 $w .= " and pay_id = ". $arr['pay_id'];
				}
			$sql = "SELECT IFNULL(SUM(order_amount),0) AS tprice FROM `{$this->App->prefix()}goods_order_info`".$w;
