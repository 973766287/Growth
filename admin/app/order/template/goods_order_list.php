...
<div class="contentbox">
	...
	?>
	<option value="<?php echo $row['pay_id'];?>"<?php echo  isset($_GET['pay_id'])&&$_GET['pay_id']==$row['pay_id'] ? ' selected="selected"' : '';?>><?php echo $row['pay_desc'];?></option>			
						<?php } ?>
		...
		<input value=" 搜索 " class="order_search" type="button">
            <label style="display: inline-block">
                <form  method="post"   name="dcs" action="goods_order.php?type=list"    > <input type="hidden" name="chaxun" value="<?php echo $chaxun ?>">
    ...
    <tr>
	   ...
	   <th><a href="<?php echo $own;?>">手续费</a></th>
	   <th><a href="<?php echo $own;?>">刷卡费率</a></th>
       <th><a href="<?php echo $own;?>">结算金额</a></th>
	...
	<td><?php echo $row['feilv2']*$row['tprice'];?></td>
	<td><?php echo $row['feilv2'];?></td>
	<td><?php echo $row['tprice']-$row['feilv2']*$row['tprice'];?></td>
	<td><?php echo $row['tprice'];?></td>
	<td>
	<?php echo $row['status'];?>
    <?php if($row['user_id'] == 42 && $row['pay_id'] == 19 && $row['pay_status'] == 1){ echo "(".$df_status.")";}?>
    </td>
  ...
  //批量删除
   ...
   $('.delorder').click(function(){
	...
	$('.liushui_search').click(function(){
		...
		$.post('<?php echo $thisurl;?>',{action:'liushui_search',add_time1:time1,add_time2:time2,pay_id:pay_id},function(data){
        ...
        if(!empty($orderlist)){
            ...
            foreach($orderlist as $row){
                ...
                <tr>
                    ...
                    <td><?php echo $i;?></td>
                    <td style='vnd.ms-excel.numberformat:@'><?php echo $row['order_sn'];?></td>
                    <td><?php echo $row['add_time'];?></td>
                    <td><font color="#FF0000">[<?php echo $row['nickname'];?>]</font><?php echo $row['consignee'];?>[真实姓名:<?php echo $row['uname'];?>]</td>
                    <td><?php echo $row['feilv2']*$row['tprice'];?></td>
                    <td><?php echo $row['feilv2'];?></td>
