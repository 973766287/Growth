...
class SystemController extends Controller{
	...
	function getcount(){
		...
		*/
		...
		//评论数
		$sql = "SELECT COUNT(comment_id) AS comcount FROM `{$this->App->prefix()}comment` WHERE parent_id='0' and  supplier_id='0' GROUP BY status ORDER BY status DESC";
		$rt['commentcount']= $this->App->findcol($sql);
		
		//商品数
		$sql = "SELECT COUNT(goods_id) FROM `{$this->App->prefix()}goods` where supplier_id = '0'";
		$rt['goods']['zcount'] = $this->App->findvar($sql);
		
		$sql = "SELECT COUNT(goods_id) FROM `{$this->App->prefix()}goods` WHERE is_on_sale='1' and supplier_id = '0'";
		$rt['goods']['sale'] = $this->App->findvar($sql);
		
		$sql = "SELECT COUNT(goods_id) FROM `{$this->App->prefix()}goods` WHERE is_on_sale='0' and supplier_id = '0'";
		$rt['goods']['no_sale'] = $this->App->findvar($sql);
		
		$sql = "SELECT COUNT(goods_id) FROM `{$this->App->prefix()}goods` WHERE is_best='1' OR is_hot='1' OR is_new='1' and supplier_id = '0'";
		$rt['goods']['promote'] = $this->App->findvar($sql);
		
		//订单数量
		$sql = "SELECT COUNT(order_id) FROM `{$this->App->prefix()}goods_order_info` where supplier_id = '0'";
		$rt['order']['zcount'] = $this->App->findvar($sql);
		
		$sql = "SELECT COUNT(order_id) FROM `{$this->App->prefix()}goods_order_info` WHERE pay_status='1' and supplier_id = '0'";
