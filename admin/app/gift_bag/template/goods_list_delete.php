...
<!--	<td><?php echo $row['market_price'];?><br />[<font color="red"><?php echo !empty($row['nickname'])?$row['nickname'] : (!empty($row['user_name'])?$row['user_name']:'金葵花商城');?></font>]</td>-->
	<td><?php echo $row['shop_price'];?></td>
	<td><?php echo $row['pifa_price'];?></td>
  	<td><?php echo !empty($row['add_time']) ? date('Y-m-d',$row['add_time']) : "无知";?></td>
	<td>
	<img src="<?php echo $this->img('icon_drop.gif');?>" title="彻底删除" alt="彻底删除" id="<?php echo $row['goods_id'];?>" class="delgoodsid"/>
	</td>
	</tr>
	<?php
	 } ?>
	<tr>
		 <td colspan="8"> <input type="checkbox" class="quxuanall" value="checkbox" />
			  <input type="button" name="button" value="彻底批量删除" disabled="disabled" class="bathdel" id="bathdel" style="cursor:pointer"/>
