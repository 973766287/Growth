	  ...
	  <tr>
		...
		<td>
		  <input name="pwd_confirm" class="pwd_confirm" maxlength="32" size="34" type="password"><span class="require-field">*</span></td>
            ...
            <tr>
		<td class="label">权限管理密码</td>
		<td>
		  <input name="priv_password" class="priv_password" maxlength="32" size="34" type="password"><span class="require-field">*</span></td>
...
//jQuery(document).ready(function($){
	...
	$('.addadmin').click(function(){
		...
		con_pass  = $('.pwd_confirm').val();
                priv_password  = $('.priv_password').val();
		emails  = $('.email').val();
		uname  = $('.adminname').val();
		grid = $('.groupid').val(); 
		...
		$.post('<?php echo $thisurl;?>',{action:'addmanmger',pass:pass,priv_password:priv_password,uname:uname,email:emails,groupid:grid,aid:aid},function(data){ 
			removewindow();
			if(data == ""){
