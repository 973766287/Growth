...
<!-- <select name="uid">
        <option value="">--选择供应商--</option>
                ...
                <option value="<?php echo $row['user_id']; ?>" <?php
                        if (isset($_GET['uid']) && $_GET['uid'] == $row['user_id']) {
                            echo 'selected="selected""';
                        }
                        ?>><?php echo $row['user_name'] . (!empty($row['nickname']) ? '&nbsp;&nbsp;[' . $row['nickname'] . ']' : ""); ?></option>
                        <?php
                    }//end foreach
                }
                ?>
                    ...
                    if (!empty($catelist)) {
                        foreach ($catelist as $row) {
                            ...
                            <option value="<?php echo $row['id']; ?>" <?php
                                    if (isset($_GET['cat_id']) && $_GET['cat_id'] == $row['id']) {
                                        echo 'selected="selected""';
                                    }
                            ...
                            if (!empty($row['cat_id'])) {
                                foreach ($row['cat_id'] as $rows) {
                                    ...
                                    <option value="<?php echo $rows['id']; ?>"  <?php
                                            if (isset($_GET['cat_id']) && $_GET['cat_id'] == $rows['id']) {
                                                echo 'selected="selected""';
                                            }
                                            ...
                                            if (!empty($rows['cat_id'])) {
                                                foreach ($rows['cat_id'] as $rowss) {
                                                    ?>
                                            <option value="<?php echo $rowss['id']; ?>"  <?php
                                            if (isset($_GET['cat_id']) && $_GET['cat_id'] == $rowss['id']) {
                                                echo 'selected="selected""';
                                            }
                                            ...
                                            if (!empty($rows['cat_id'])) {
                                                foreach ($rowss['cat_id'] as $rowsss) {
                                                    ?>
                                                    <option value="<?php echo $rowsss['id']; ?>"<?php
                                if (isset($_GET['cat_id']) && $_GET['cat_id'] == $rowsss['id']) {
                                    echo 'selected="selected""';
                                }
                                                    ?>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $rowsss['name']; ?></option>
        ...
        ?>
                ...
                <select name="is_goods_attr">
                    ...
                    <option value="is_on_sale1" <?php
                    if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_on_sale1') {
                        echo 'selected="selected""';
                    }
                    ?>>上架</option>
                    <option value="is_on_sale0" <?php
                    if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_on_sale0') {
                        echo 'selected="selected""';
                    }
                    ?>>下架</option>

                    <option value="is_hot" <?php
                    if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_hot') {
                        echo 'selected="selected""';
                    }
                    ?>>热销</option>
                    <option value="is_new" <?php
                    if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_new') {
                        echo 'selected="selected""';
                    }
                    ?>>新品</option>
                    <option value="is_best" <?php
                    if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_best') {
                        echo 'selected="selected""';
                    }
                    ?>>精品</option>
                    <!--			<option value="is_alone_sale" <?php
                    if (isset($_GET['is_alone_sale']) && $_GET['is_alone_sale'] == 'is_alone_sale') {
                        echo 'selected="selected""';
                    }
                    ?>>红包</option>
                    -->			<option value="is_promote" <?php
                            if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_promote') {
                                echo 'selected="selected""';
                            }
                            ?>>特价商品</option>
                    <!--<option value="is_qianggou" <?php
                            if (isset($_GET['is_goods_attr']) && $_GET['is_goods_attr'] == 'is_qianggou') {
                                echo 'selected="selected""';
                            }
                            ?>>抢购商品</option>
                    <option value="is_jifen" <?php
                    if (isset($_GET['is_jifen']) && $_GET['is_jifen'] == 'is_jifen') {
                        echo 'selected="selected""';
                    }
                    ?>>积分商品</option>-->
                </select>
                ...
                <select name="brand_id">
                    ...
                    if (!empty($brandlist)) {
                        foreach ($brandlist as $row) {
                            ?>
                            <option value="<?php echo $row['id']; ?>" <?php
                            if (isset($_GET['brand_id']) && $_GET['brand_id'] == $row['id']) {
                                echo 'selected="selected""';
                            }
                            ?>><?php echo $row['name']; ?></option>
        <?php
        if (!empty($row['brand_id'])) {
            foreach ($row['brand_id'] as $rows) {
                ?>
                                    <option value="<?php echo $rows['id']; ?>"  <?php
                if (isset($_GET['brand_id']) && $_GET['brand_id'] == $rows['id']) {
                    echo 'selected="selected""';
                }
                ?>>&nbsp;&nbsp;<?php echo $rows['name']; ?></option>
                <?php
                if (!empty($rows['brand_id'])) {
                    foreach ($rows['brand_id'] as $rowss) {
                        ?>
                                            <option value="<?php echo $rowss['id']; ?>"  <?php
                        if (isset($_GET['brand_id']) && $_GET['brand_id'] == $rowss['id']) {
                            echo 'selected="selected""';
                        }
                        ?>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $rowss['name']; ?></option>
 ...
 <!--	   <th><a href="<?php echo $is_on_sale; ?>">上架</a></th>-->
    ...
    if (!empty($rt)) {
        foreach ($rt as $row) {
            ...
            <!--	<td><span class="vieworder" id="<?php echo $row['goods_id'] . '-' . $row['suppliers_id']; ?>" lang="market_price"><?php echo $row['market_price']; ?></span><br />[<font color="red"><?php echo!empty($row['nickname']) ? $row['nickname'] : (!empty($row['user_name']) ? $row['user_name'] : '金葵花商城'); ?></font>]</td>-->
