...
<div class="contentbox">
	 ...
	 <tr>
		<th colspan="15" align="left"><span style="float:left">实体商品列表</span><a href="<?php echo ADMIN_URL.'goods.php?type=goods_info';?>" style="float:right; padding:2px 4px 2px 4px; background:#ededed; border-bottom:2px solid #ccc; border-right:2px solid #ccc">添加商品</a></th>
	</tr>
	<tr><td colspan="15" align="left" class="label">
...
<!--			<option value="is_alone_sale" <?php if(isset($_GET['is_alone_sale'])&&$_GET['is_alone_sale']=='is_alone_sale'){ echo 'selected="selected""'; } ?>>红包</option>
		...
		<?php
         ...
         <!--{审核状态zzzzzzzzzzzzzzzzzzzz}-->
         <select name="sale">
         <option>审核状态</option>
          <option value="0" <?php if(isset($_GET['sale'])&&$_GET['sale']==0){ echo 'selected="selected""'; } ?>>未审核</option>
           <option value="1" <?php if(isset($_GET['sale'])&&$_GET['sale']==1){ echo 'selected="selected""'; } ?>>审核通过</option>
            <option value="-1" <?php if(isset($_GET['sale'])&&$_GET['sale']==-1){ echo 'selected="selected""'; } ?>>审核未通过</option>
...
<td><? if($row['is_check']==-1){echo "审核未通过";}else{ ?><img src="<?php echo $this->img($row['is_check']==1 ? 'yes.gif' : 'no.gif');?>" alt="<?php echo $row['is_check']==1 ? '0' : '1';?>" class="activeop" lang="is_check" id="<?php echo $row['goods_id'];?>"/><? }?></td>
...
<!----------------- look添加 结束 ----------------------------------------------------------->	
	...
	$('.cate_search').click(function(){
		...
		location.href='<?php echo $thisurl;?>?type=goods_lists&uid='+u_id+'&cat_id='+catid+'&is_goods_attr='+is_goods+'&brand_id='+bid+'&keyword='+keys+'&is_delete=0&sale='+sale;
	});
