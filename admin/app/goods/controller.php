...
class GoodsController extends Controller {
    ...
    function __construct() {
        ...
        $url = ADMIN_URL . 'goods.php?' . $_SERVER['QUERY_STRING'];
        $this->set("url", $url);
    }
    ...
    function goods_tuijian($data = array()) {
        ...
        if ($id > 0) {
            ...
            $this->jump(ADMIN_URL . 'goods.php?type=goods_tuijian');
            exit;
        }
    ...
    function goods_info($gid = 0) {
        $url_href = ADMIN_URL . 'goods.php?type=goods_list';
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $_POST['is_prize'] = isset($_POST['is_prize']) && intval($_POST['is_prize']) > 0 ? intval($_POST['is_prize']) : '0';
			...
			if(isset($_POST['supplier_id']) && intval($_POST['supplier_id']) > 0){
				...
				$_POST['is_check'] = isset($_POST['is_check']) ? $_POST['is_check'] : '0';
			}else{
			$_POST['is_check'] = '1';
			}
    ...
    function goods_spend_gift() {
        ...
        if (isset($_GET['tt']) && !empty($_GET['tt'])) {
            if ($_GET['tt'] == 'del') {
                ...
                $cache->write($fn, $spendgift, 'spendgift');
                $this->jump('goods.php?type=spend_gift');
                exit;
            } elseif ($_GET['tt'] == 'update') {
                $id = $_GET['id'];
                $this->jump('goods.php?type=spend_gift&id=' . $id);
                exit;
            }
        }
        if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    function ajax_get_group_goods($data = array()) {
        ...
        if ($bid > 0)
            $comd[] = 'tb1.brand_id=' . intval($bid);

        if (!empty($key))
            $comd[] = "(tb1.goods_name LIKE '%" . trim($key) . "%')";
    ...
    function cate_info($cid = 0) {
         $url_href = ADMIN_URL . 'goods.php?type=cate_list';
        $this->check_priv($url_href);
    ...
    function goods_comment_list($uid = 0, $oid = 0) {
        ...
        $list = 12;
        $start = ($page - 1) * $list;
        $sql = "SELECT COUNT(comment_id) FROM `{$this->App->prefix()}comment` WHERE supplier_id = '0' and parent_id='0'";
        $tt = $this->App->findvar($sql);
        $pagelink = Import::basic()->getpage($tt, $list, $page, '?page=', true);
        $this->set("pagelink", $pagelink);
        ...
        $sql .= " WHERE c.supplier_id = '0' and c.parent_id ='0' $u $orderby LIMIT $start,$list";
        $rt = $this->App->find($sql);
        // print_r($rt);
    ...
    function goods_comment_info($id = 0) {
         $url_href = ADMIN_URL . 'goods.php?type=comment_list';
        $this->check_priv($url_href);
        if (empty($id)) {
    ...
    function goods_attr_info($id = 0) {
          $url_href = ADMIN_URL . 'goods.php?type=goods_attr_list';
        $this->check_priv($url_href);
        $rt = array();
        if (isset($_POST) && !empty($_POST)) {
        ...
        foreach ($id_arr as $id) {
            if (Import::basic()->int_preg($id)) {
                if ($this->App->delete('goods', 'goods_id', $id)) { //删除商品
                    ...
                    $this->App->delete('goods_gift', 'goods_id', $id); //删除商品红包
    ...
    function ajax_delgoods_suppliers($data = array()) {
        ...
        if (isset($data['type']) && $data['type'] == 'check') {
            ...
            $sql = "UPDATE `{$this->App->prefix()}goods` SET is_check = '1' WHERE goods_id IN(" . @implode(',', $id_arr) . ")";
            $this->App->query($sql);
    ...
    function ajax_del_subcate($data = array()) {
        if (empty($data['cid']) || empty($data['gid']))
            die("传送的ID为空！");
        $cid = $data['cid'];
        $gid = $data['gid'];
...
//zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz
  ...
  //入驻商商品审核
  //  function ajax_activeops($data = array()) {
//        if (empty($data['gid']))
//            die("非法操作，ID为空！");
//        $type = $data['type'];
//        switch ($type) {
...
//zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz
    ...
    function ajax_activeop_suppliers($data = array()) {
        ...
        $this->App->update('goods', $sdata, array("goods_id='$ggid'"));
