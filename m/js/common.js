...
function createwindow(){
	...
	h = getScrollTop();
	var str = '<div class="openwindow"><img src="'+SITE_URL+'images/loadings.gif"  align="absmiddle"/>';
	$("body").append(str);
	$('.openwindow').css('position','absolute');
	$('.openwindow').css('left',((screen.availWidth-200)/2)+'px');
	//$('.openwindow').css('left','30px');
...
function createwindow_instead(){
	obj = $('.openwindow');
	if(typeof(obj)!='undefined') $(obj).remove();
	h = getScrollTop();
	var str = '<div class="openwindow"><img src="'+SITE_URL+'images/loadings.gif"  align="absmiddle"/></div><div id="fade" class="black_overlay"></div>';
	$("body").append(str);
	$('.openwindow').css('position','absolute');
	$('.openwindow').css('left',((screen.availWidth-200)/2)+'px');
	//$('.openwindow').css('left','30px');
...
function clearTip(obj) {
  ...
  if(obj.value=="输入搜索关键字" || obj.value=="联系人姓名不能为空" || obj.value=="联系电话不能为空" || obj.value=="请填写就餐时间" || obj.value=="根据实际情况可以填写你想说的") {
