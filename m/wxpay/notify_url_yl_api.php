<?php
	...
	$sign = $_REQUEST['sign'];
	   ...
	   if($_REQUEST['errCode'] == '00'){
			...
			sleep(3);
			if($app->action('shopping','query_pay_record',$order_no)){
					   		   
	  error_log('['.date('Y-m-d H:i:s').']语句:'."\n". var_export($_POST,true)."\n\n", 3, '../app/shopping/pay_successs_status_api/ys_pay_'.date('Y-m-d').'.log');
	  
	  
	echo "success";
				}else{
					 error_log('['.date('Y-m-d H:i:s').']语句:'."\n". var_export($_POST,true)."\n\n", 3, '../app/shopping/pay_successs_status_api/new_pay_'.date('Y-m-d').'.log');
