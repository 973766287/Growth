...
<div id="main" style="padding-top:0px; min-height:300px">
	...
	<form action="<?php echo ADMIN_URL;?>mycart.php?type=confirm" method="post" name="CONSIGNEE_ADDRESS" id="CONSIGNEE_ADDRESS">
		...
		<?php $userress_id = 0; foreach($rt['userress'] as $row){?>
         ...
         <? if(!empty($dianpus)){ foreach ($dianpus as $_k){?>
  
  <input type="hidden" name="supp[]" value="<? echo $_k;?>"/>
<p style="border-bottom:2px solid #CCC" >店铺：<? echo $this->action('shopping','ajax_get_suppname',array('supplier_id'=>$_k));?>商品清单</p>
         ...
         <? if($row['supplier_id'] == $_k){?>
			<tr>
				...
				<p style="padding-left:10px; padding-right:36px; position:relative; line-height:18px;">
					...
					<?php if(!empty($row['buy_more_best'])){echo '<br />该商品实行<font style="color:#FE0000;font-weight:bold">['.$row['buy_more_best'].']</font>促销活动，欢迎订购！';}?><span style="padding:2px 5px 2px 5px; color:#FF0000; cursor:pointer; position:absolute; right:5px; top:0px; z-index:22; background:#fafafa; border:1px solid #ededed;border-radius:5px;" class="delcartid" id="<?php echo $k;?>">删除</span></p>
				<?php if(!empty($row['spec'])){
				 echo '<p style="padding-left:10px;">'.implode("、",$row['spec']).'</p>';
				 } ?>
				 <p style=" padding-left:10px;font-size:12px;line-height:20px;" class="raturnprice raturnprice<?php echo $k;?>">专柜价:<font color="#5f5f5f">￥<?php echo $row['shop_price'];?></font>&nbsp;&nbsp;惊喜价:<font color="#FF0000" class="gprice<?php echo $k;?>">￥<?php echo $row['price']>0 ? $row['price']  : $row['pifa_price'];?></font></p>
				 <div class="item" style="height:20px; line-height:20px; position:relative; padding-left:10px; padding-top:7px">
                                     <?php if($row['is_prize']!=1){?>
						<a class="jian" style="cursor:pointer; display:block; float:left; width:35px; height:22px;line-height:22px;text-align:center; font-size:18px; font-weight:bold; border:1px solid #ccc; background:#ededed">-</a>
                                     <?php } ?>
                                                <input readonly="" id="<?php echo $k;?>" name="goods_number" value="<?php echo $row['number'];?>" class="inputBg" style="float:left;text-align: center; width:20px; height:22px; line-height:22px;border-bottom:1px solid #ccc; border-top:1px solid #ccc" type="text">
                                             <?php if($row['is_prize']!=1){?>
                                                <a class="jia" style="cursor:pointer; display:block; float:left; width:35px; height:22px;line-height:22px;text-align:center; font-size:18px; font-weight:bold; border:1px solid #ccc; background:#ededed">+</a><b style="margin-left:3px;"><?php  echo $row['goods_unit'];?></b>
						     <?php } ?>
        ...
        <table border="0" cellpadding="0" cellspacing="0" style="width:100%;border-bottom:1px solid #ededed; margin-top:10px;">
          <? if($_k != 0){?>
     <input type="hidden" name="free_<? echo $_k;?>"  id="free_<? echo $_k;?>" value="0"/>
...
//					foreach($rt['shippinglist'] as $k=>$row){
					?>
					   <td><label><span><input onclick="return jisuan_shopping('<?php echo $row['shipping_id'];?>','<? echo $_k;?>')" name="shipping_id_<? echo $_k;?>" id="shipping_id_<? echo $_k;?>" value="<?php echo $row['shipping_id'];?>" type="radio" /></span><strong><?php echo $row['shipping_name'];?></strong></label>
					  <?php 
						$f = $this->action('shopping','ajax_jisuan_shopping',array('shopping_id'=>$row['shipping_id'],'userress_id'=>($userress_id > 0 ? $userress_id : '5') ,'supplier_id'=>$_k),'cart');
						$f = $f>0 ? $f : '0.00';
						$free[] = $f;
						?>
					  </td>
...
//					foreach($rt['shippinglist'] as $k=>$row){
					?>
					   <td><label><span><input onclick="return jisuan_shopping('<?php echo $row['shipping_id'];?>','<? echo $_k;?>')" name="shipping_id_0" id="shipping_id_0" value="<?php echo $row['shipping_id'];?>" type="radio" /></span><strong><?php echo $row['shipping_name'];?></strong></label>
					  <?php 
						$f = $this->action('shopping','ajax_jisuan_shopping',array('shopping_id'=>$row['shipping_id'],'userress_id'=>($userress_id > 0 ? $userress_id : '5'),'supplier_id'=>$_k),'cart');
						$f = $f>0 ? $f : '0.00';
						$free[] = $f;
...
<!--			<p style="height:20px; line-height:20px;">
			...
			<p style="line-height:22px; color:#222; font-size:14px; font-weight:bold; color:#9A0000; padding-top:5px;">总金额:￥<span class="ztotals"><?php echo $zp;?></span>+￥<span class="freeshopp">0.00</span>(邮费)=<span class="freeshoppandprice"><?php echo $zp;?></span>元</p>
...
/*	ss = $('input[name="shipping_id"]:checked').val(); 
	...
	<? foreach($dianpus as $_k){
		
		?>
		
		sss = $('input[name="shipping_id_<? echo $_k?>"]:checked').val(); 
	if(typeof(sss)=='undefined' || sss ==""){
		alert("请选择配送方式！");
		return false;
	}
...
//计算邮费
function jisuan_shopping(id,sid){
		...
		/*$.post('<?php echo $thisurl;?>',{action:'jisuan_shopping',shopping_id:id,userress_id:uu,supplier_id:sid},function(data){
				...
				//	alert(data);
					...
					if(arr.length==3){
						...
						document.getElementById("free_"+sid).value = arr[1];
						
						free=0;
						<? foreach($dianpus as $_k){?>
						free_<? echo $_k;?> = document.getElementById("free_<? echo $_k;?>").value; 
						
						free = parseFloat(free) + parseFloat(free_<? echo $_k;?>);
						<? }?>
						
					$('.free_shopping').html('￥'+free);
					b = $('.nototals').html();
					$('.ztotals').html(parseFloat(b)+parseFloat(free));
					...
					document.getElementById("free_"+sid).value = 0;
		...
		$.post('<?php echo $thisurl;?>',{action:'jisuan_shopping',shopping_id:id,userress_id:uu,supplier_id:sid},function(data){
				if(data !="" && typeof(data) !='undefined'){
					...
					$('.freeshoppandprice').html(toDecimal(parseFloat(b)+parseFloat(free)));
					}else{
						alert(data);
					}
				}else{
					...
					$('.freeshoppandprice').html(parseFloat(b)+parseFloat(free));
				}
				removewindow();
		});
