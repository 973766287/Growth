  ...
  <div class="add">
	...
	<table width="100%" border="0" style="line-height:24px;padding:10px;background:#f5f5f5;" cellpadding="0" cellspacing="0">
	  <tr>
		...
		<tr>
		<td align="left"><input name="consignee" class="pw"  value="<?php echo !empty($rt['userressinfo']['consignee'])?$rt['userressinfo']['consignee']:'';?>" type="text"></td>
	  </tr>
	  <tr>
	...
	?>
	<option value="<?php echo $row['region_id'];?>" <?php echo $rt['userressinfo']['province']==$row['region_id']? 'selected="selected"' :"";?>><?php echo $row['region_name'];?></option>	
	<?php } } ?>													
	</select>
	...
	?>
	<option value="<?php echo $row['region_id'];?>" <?php echo $rt['userressinfo']['city']==$row['region_id']? 'selected="selected"' :"";?>>
            <?php echo $row['region_name'];?></option>	
	<?php } } ?>	
	</select>
	
	<select <?php echo !isset($rt['userressinfo']['district'])? 'style="display: none;"':"";?> name="district" class="pwt" id="select_district">
	<option value="0">选择区</option>	
	<?php 
	if(!empty($rt['district'])){
	foreach($rt['district'] as $row){
	?>
	<option value="<?php echo $row['region_id'];?>" <?php echo $rt['userressinfo']['district']==$row['region_id']? 'selected="selected"' :"";?>><?php echo $row['region_name'];?></option>	
	<?php } } ?>													
	</select>
	</td>
	  </tr>
	  ...
	  <tr>
		<td align="left"><input name="address" class="pw" value="<?php echo !empty($rt['userressinfo']['address'])?$rt['userressinfo']['address']:'';?>" type="text"/></td>
	  </tr>
	  <tr>
		<td align="left"><b class="cr2">*</b> 邮箱：</td>
	  </tr>
	  <tr>
		<td align="left"><input type="text" class="pw" name="email" value="<?php echo !empty($rt['userressinfo']['email'])?$rt['userressinfo']['email']:'';?>" /></td>
	  </tr>
	  <tr>
		<td align="left"><b class="cr2">*</b> 电话：</td>
		</tr>
		<tr>
		<td align="left"><input type="text" class="pw" name="mobile" value="<?php echo !empty($rt['userressinfo']['mobile'])?$rt['userressinfo']['mobile']:'';?>"/></td>
	  </tr>
	  <tr>
		<td align="center" style="padding-top:10px;">
                    <?php if(isset($rt['iid']) and !empty($rt['iid'])){?>
                 
		<a href="javascript:;" onclick="ressinfoop('<?php echo $rt['iid'];?>','update','CONSIGNEE_ADDRESS')" style="display:block;background:#e13935;cursor:pointer;width:110px; height:24px; line-height:24px; font-size:14px; color:#FFF; font-weight:bold; text-align:center;border-radius: 5px;overflow:hidden">确定修改</a>
                    <?php } else{?>
                <a href="javascript:;" onclick="ressinfoop('0','add','CONSIGNEE_ADDRESS')" style="display:block;background:#e13935;cursor:pointer;width:110px; height:24px; line-height:24px; font-size:14px; color:#FFF; font-weight:bold; text-align:center;border-radius: 5px;overflow:hidden">确定添加</a>
                <?php  }?>
                </td>
	  </tr>
	</table>
