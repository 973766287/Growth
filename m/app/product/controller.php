...
class ProductController extends Controller {
    ...
    function index($gid = 0) {
        ...
        $is_prize = 0;
        $has = 0;
        if ($rt['goodsinfo']['is_prize'] == '1') {
            $this->mycart_clear();
            $is_prize = 1;
            //判断当前用是否会员
            $uid = $this->Session->read('User.uid');
            if (!$uid) {
                //  $this->jump(ADMIN_URL, 0, '您暂时无法购买此产品');   exit;
                $has = 2; //请先登录
            }
//s扫描别人二维码进来的
           /* if ($uid > 0) {
...
//查询所有奖品区的商品
        ...
        if ($rt['goodsinfo']['is_promote'] == '1' && $rt['goodsinfo']['promote_start_date'] < mktime() && $rt['goodsinfo']['promote_end_date'] > mktime()) {
            $rt['goodsinfo']['is_promote'] == '1';
        } else {
            $rt['goodsinfo']['is_promote'] == '0';
        }
        if ($rt['goodsinfo']['is_qianggou'] == '1' && $rt['goodsinfo']['qianggou_start_date'] < mktime() && $rt['goodsinfo']['qianggou_end_date'] > mktime()) {
            $rt['goodsinfo']['is_qianggou'] == '1';
        } else {
            $rt['goodsinfo']['is_qianggou'] == '0';
        }
    ...
    function ajax_add_cart($data = array()) {
        ...
        if ($price > 0)
            $cart['pifa_price'] = $price; //不同属性的价格改变
