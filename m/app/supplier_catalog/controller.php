...
class Supplier_catalogController extends Controller {
    ...
    function index($cid = 0, $page = 0) {
        ...
        if (is_null($rt)) {
            ...
            $cid = (isset($_GET['cid']) && intval($_GET['cid']) > 0) ? intval($_GET['cid']) : $cid;
			  $gid = (isset($_GET['suppId']) && intval($_GET['suppId']) > 0) ? intval($_GET['suppId']) : $cid;
            ...
            //条件
            $comd = array('cid' => $cid, 'bid' => $bid, 'price' => $price, 'keyword' => $keyword, 'supplier_id' => $gid); //需要的话继续增加
       ...
       $this->set('info', $info);
            ...
            $this->template($mb . '/supplier_catalog_index');
