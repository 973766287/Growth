...
class Supplier_catalogModel extends Model {
    ...
    function __get_goods_count_category($data = array()) {
        ...
        $sql .=" LEFT JOIN `{$this->prefix()}supplier_category` AS gc ON gc.cat_id = g.cat_id";
...
//echo $sql;
    ...
    function __category_goods_where($data = array()) {
        ...
        $supplier_id = isset($data['supplier_id']) && intval($data['supplier_id']) > 0 ? intval($data['supplier_id']) : 0;
        ...
        if ($cid > 0) { //子分类
           ...
           // $comd[] = "(g.supplier_cat_id" . db_create_in($sub_cid) . " OR csg.supplier_cat_id='$cid')";
		    $comd[] = "(g.supplier_cat_id" . db_create_in($sub_cid) . ")";
        }

        $comd[] = "g.is_on_sale = '1' AND g.is_check='1' AND g.is_delete = '0' AND g.is_alone_sale='1' AND g.is_jifen='0'";

        ($supplier_id > 0) ? $comd[] = "g.supplier_id='$supplier_id'" : ""; //品牌
        ...
        if (!empty($keyword)) {
            $act = array('is_best', 'is_new', 'is_hot', 'is_promote', 'is_qianggou','is_prize');
            if (in_array($keyword, $act)) {
                switch ($keyword) {
                    case 'is_best':
                        $comd[] = "g.is_best='1' and g.is_prize='0'";
                        break;
                    case 'is_new':
                        $comd[] = "g.is_new='1' and g.is_prize='0' ";
                        break;
                    case 'is_hot':
                        $comd[] = "g.is_hot='1' and g.is_prize='0'";
                        break;
                    case 'is_prize':
                        $comd[] = "g.is_prize='1'";
                        break;
                    case 'is_promote':
                        $time = mktime();
                        $comd[] = "AND g.is_promote = '1' AND g.promote_start_date <= '$time' AND g.promote_end_date >= '$time' and g.is_prize='0'";
                    case 'is_qianggou':
                        $time = mktime();
                        $comd[] = "AND g.is_qianggou = '1' AND g.qianggou_start_date <= '$time' AND g.qianggou_end_date >= '$time' and g.is_prize='0'";
                        break;
                }
            } else {
    ...
    function get_goods_cate_tree_model($cid = 0) {
        $three_arr = array();
        $sql = 'SELECT count(cat_id) FROM `' . $this->prefix() . "supplier_category` WHERE parent_id = '$cid' AND is_show = 1";
        if ($this->findvar($sql) || $cid == 0) {
            $sql = 'SELECT tb1.cat_name,tb1.cat_id,tb1.parent_id,tb1.is_show,tb1.cat_title,tb1.cat_desc, tb1.keywords,tb1.show_in_nav,tb1.sort_order, COUNT(tb2.cat_id) AS goods_count FROM `' . $this->prefix() . "supplier_category` AS tb1";
