...
class PageController extends Controller {
        ...
        if (file_exists($fn) && !$cache->GetClose()) {
            ...
            if (!empty($rt['cat']))
                foreach ($rt['cat'] as $row) {
                    ...
                    $sql = "SELECT goods_id,goods_name,goods_thumb,goods_img,pifa_price,shop_price,sale_count FROM `{$this->App->prefix()}goods` WHERE (cat_id ='$cid' or cat_id in (select cat_id from gz_goods_cate where parent_id='$cid')) AND is_on_sale='1' AND is_jifen = '0' AND is_delete = '0'  and is_prize=0  ORDER BY sort_order ASC LIMIT 6";
                    $rt['goods'][$row['cat_id']] = $this->App->find($sql);
                }

            //推荐产品
            $sql = "SELECT goods_id,goods_name,goods_thumb,goods_img,pifa_price,shop_price,sale_count FROM `{$this->App->prefix()}goods` WHERE is_best='1' AND is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0  and is_best ='1' ORDER BY sort_order ASC, supplier_id asc, goods_id DESC LIMIT 6";
            ...
            //$rt['tj'] = $this->App->findrow($sql);
            //统计
            $sql = "SELECT COUNT(goods_id) FROM `{$this->App->prefix()}goods` WHERE is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0 LIMIT 1";
    ...
    function coupon() {
        ...
        if ($type_id > 0) {
            ...
            $this->jump(ADMIN_URL . 'sp.php?type=coupon&id=' . $shopid);
            exit;
        }
    ...
    function couponinfo() {
        ...
        if (!empty($_POST)) {
            if (empty($_POST['type_name'])) {
                $this->jump('', 0, '类型名称不能为空！');
                exit;
            }
            if (!($_POST['type_money'] > 0)) {
                $this->jump('', 0, '红包金额必须大于0！');
                exit;
            }
    ...
    function couponsend() {
        ...
        if (isset($_POST) && !empty($_POST['bonus_sum'])) {
            ...
            $this->jump(ADMIN_URL . 'sp.php?type=couponview&shopid=' . $shopid . '&type_id=' . $type_id);
            exit;
        }
    ...
    function couponview() {
        ...
        if (isset($_GET['op']) && $_GET['op'] == 'del' && intval($_GET['delid']) > 0) { //删除
            ...
            if ($this->App->delete('user_coupon_list', 'bonus_id', intval($_GET['delid']))) {
                $this->jump('sp.php?type=couponview&shopid=' . $shopid . '&type_id=' . $id);
                exit;
            }
        }
        $list = 15;
    ...
    function photosinfo() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            if ($id > 0) { //编辑
                ...
                $this->jump(SITE_URL . 'shop/sp.php?type=photosinfo&id=' . $id, 0, '修改成功！');
                exit;
            } else { //添加
                $_POST['shop_id'] = $uid;
        ...
        if ($cid > 0) { //编辑页面
            if (isset($_POST) && !empty($_POST)) {
                if (empty($_POST['cat_name'])) {
                    ...
                    $this->jump(ADMIN_URL . 'sp.php?type=shopcateinfo&id=' . $shopid, 0, '编辑成功！');
                    exit;
                    // }
                }
                $rt = $_POST;
            ...
            if (isset($_POST) && !empty($_POST)) {
                if (empty($_POST['cat_name'])) {
                    ...
                    $this->jump(ADMIN_URL . 'sp.php?type=shopcateinfo', 0, '添加成功！');
                    exit;
                    // }
                }
                $rt = $_POST;
    ...
    function userxiaofei() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if (!empty($key)) {
                ...
                if ($uid != $uids) {
                    $this->jump(SITE_URL . 'shop/sp.php?type=userxiaofei&id=' . $uids);
                    exit;
                }
            }
            ...
            if ($tt == 1) {
                $this->jump(SITE_URL . 'shop/sp.php?type=userxiaofei&id=' . $uid, 0, '成功消费！');
                exit;
            }
        }
        $rt = array();
    ...
    function ajax_del_subcate($data = array()) {
        if (empty($data['cid']) || empty($data['shopid']))
            die("传送的ID为空！");
        $cid = $data['cid'];
        $shopid = $data['shopid'];
