...
class CatalogController extends Controller {
    ...
    function index($cid = 0, $page = 0, $rs = array()) {
        ...
        if (is_null($rt)) {
            if (empty($_GET['encode'])) {
                if (isset($_GET['keyword']) && !empty($_GET['keyword']) && !in_array($_GET['keyword'], array('is_best', 'is_new', 'is_hot', 'is_promote', 'is_prize'))) {
            ...
            if ($cid > 0) {
                ...
                switch ($keyword) {
                    ...
                    case 'is_promote':
                        ...
                        break;
                    case 'is_prize':
                        $hear[] = '<a href="' . SITE_URL . 'prize/">奖品区</a>';
                        break;
                    default:
        ...
        $this->set('page', $page);
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        if (!empty($keyword) and $keyword == 'is_prize') {
            //查询其他赠品礼包
            $sql = "SELECT * FROM `{$this->App->prefix()}user_level`  where  lid in (10,11,12) ";
            $userrank = $this->App->find($sql);
            foreach ($userrank as $_k => $_v) {
                $sql = "SELECT * FROM `{$this->App->prefix()}gift_bag`  where  type ='$_v[lid]' ";
                $bag = $this->App->find($sql);
                foreach($bag as $_k1=>$_v1){
                  $bag[ $_k1]['goods_thumb'] = is_file(SYS_PATH . $_v1['goods_thumb']) ? SITE_URL . $_v1['goods_thumb'] : SITE_URL . 'theme/images/no_picture.gif';
                }
                  $userrank[$_k]['bag'] = $bag;
                unset($bag);
            }
            ...
            $this->template($mb . '/catalog_prize');
        } else {
 ...
 foreach ($rt['categoodslist'] as $row) {
                    ...
                    $sql = "SELECT goods_id,goods_name,goods_thumb,goods_img,pifa_price,shop_price,sale_count FROM `{$this->App->prefix()}goods` WHERE supplier_id = $sid AND is_on_sale='1' AND is_jifen = '0' AND is_delete = '0'  and is_prize=0 and is_shop_best=1  ORDER BY sort_order ASC LIMIT 3";
					
					
                    $rt['goods'][$row['supplier_id']] = $this->App->find($sql);
	  ...
	  $title = "店铺街";
	   if (!defined(NAVNAME))
            define('NAVNAME', $title);
			
	   $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->set('mubanid', $GLOBALS['LANG']['mubanid']);
      
		
	  $this->template($mb .'/street');
