...
class CategorysController extends Controller {
    ...
    function index($cid = 0, $page = 0, $rs = array()) {
        ...
        if (is_null($rt)) {
            ...
            //end 当前位置
            //商品分类列表		
            $rt['menu'] = $this->get_goods_cate_tree($suppId);
            $rt['menu_show'] = array();
            if ($cid > 0) {
                $rt['menu_show'] = $this->get_goods_cate_tree($cid,$suppId);
                if (empty($rt['menu_show'])) {
                    $c = $this->App->findvar("SELECT parent_id FROM `{$this->App->prefix()}supplier_category` WHERE cat_id='$cid'");
    ...
    function get_goods_cate_tree($cid = 0,$suppId=0) {
        $three_arr = array();
        $sql = 'SELECT count(cat_id) FROM `' . $this->App->prefix() . "supplier_category` WHERE parent_id = '$cid' AND is_show = 1";
