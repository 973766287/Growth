...
class CategorysController extends Controller {
    ...
    function index($cid = 0, $page = 0, $rs = array()) {
		 ...
		 $suppId = (isset($_GET['suppId']) && intval($_GET['suppId']) > 0) ? intval($_GET['suppId']) : 0;
        ...
        if (is_null($rt)) {
            if (empty($_GET['encode'])) {
                if (isset($_GET['keyword']) && !empty($_GET['keyword']) && !in_array($_GET['keyword'], array('is_promote', 'is_shop_best', 'is_shop_new', 'is_shop_hot', 'is_qianggou'))) {
                    $string = array_merge($_GET, $_POST);
                    $string['search_encode_time'] = time();
                    $string = str_replace('+', '%2b', base64_encode(serialize($string)));

                    header("Location: " . get_url('商品中心', $cid, ROOT_URL . "catalog.php", 'goodscate', array($this->__module)) . "?encode=$string\n");
            ...
            //当前分类的基本信息
            if ($cid > 0) {
                $sql = "SELECT * FROM `{$this->App->prefix()}supplier_category` WHERE cat_id='$cid' LIMIT 1";
            ...
            if ($cid > 0) {
                $hear[] = '<a href="' . get_url('商品中心', 0, SITE_URL . "catalog.php", 'goodscate', array('catalog', 'index')) . '">商品中心</a>';
                ...
                if (!empty($rts)) {
                    ...
                    foreach ($rts as $rows) {
                        $hear[] = '<a href="' . get_url($rows['cat_name'], $rows['cat_id'], "catalog.php?cid=" . $rows["cat_id"], 'goodscate', array('catalog', 'index', $rows['cat_id'])) . '">' . $rows['cat_name'] . '</a>';
                    }
                }
                unset($rts, $rts_);
            } elseif (!empty($keyword)) {
                $perend_id = -1;
                $hear[] = '<a href="' . SITE_URL . 'catalog.php">商品中心</a>';
                switch ($keyword) {
                    case 'is_shop_hot':
                        $hear[] = '热销商品';
                        break;
            ...
            //end 当前位置
            //商品分类列表		
            $rt['menu'] = $this->get_goods_cate_tree(0,$suppId);
            $rt['menu_show'] = array();
            if ($cid > 0) {
                $rt['menu_show'] = $this->get_goods_cate_tree($cid,$suppId);
                if (empty($rt['menu_show'])) {
                    $c = $this->App->findvar("SELECT parent_id FROM `{$this->App->prefix()}supplier_category` WHERE supplier_id = '$suppId' and  cat_id='$cid'");
                    $rt['menu_show'] = $this->get_goods_cate_tree($c,$suppId);
                }
            } else {
                $rt['menu_show'] = $this->get_goods_cate_tree($cid,$suppId);
            }
            ...
            //子分类CID
            $sub_cids = $this->get_goods_sub_cat_ids($cid,$suppId);

            //热卖前10个商品
            //$rt['top10'] = $this->top10($sub_cids,4); 
            //$rt['menu_show'] = $this->get_goods_cate_tree();
            ...
            //排序
            //定义能够排序的字段
            $order = array('sort_order', 'cat_id', 'goods_id', 'click_count', 'brand_id', 'shop_price', 'market_price', 'promote_price', 'is_on_sale', 'is_shop_best', 'is_shop_new', 'is_shop_hot', 'is_promote', 'sale_count', 'add_time', 'last_update');
            $orderby = "";
            if (isset($_GET['desc'])) {
                ...
                $orderby = ' ORDER BY g.sort_order ASC ';
            ...
            //条件
            $comd = array('cid' => $cid, 'bid' => $bid, 'price' => $price, 'keyword' => $keyword, 'supplier_id' => $suppId); //需要的话继续增加

            $list = intval($list) > 0 ? intval($list) : 50; //每页显示多少个
            $start = ($page - 1) * $list;
            $tt = $this->App->__get_goods_count_category($comd); //获取商品的数量
            $rt['goods_count'] = $tt;
            $rt['categoodspage'] = Import::basic()->ajax_page($tt, $list, $page, 'get_categoods_page_lists', array($suppId,$cid, $bid, $price, $order_type, $sort_type, $list));
		...
		$mb = $info['moban'];
		 $this->set("mb", $mb);
		   ...
		   //PC端首页banner广告位 
        $sql = "SELECT * FROM `{$this->App->prefix()}supplier_ad_content` WHERE  is_show='1' and supplier_id={$suppId}  LIMIT 2";
...
//			 $this->set("s_info", $s_info);
			 ...
			 //所有分类
        $catList = $this->get_goods_cate_tree(0,$suppId);
 ...
 $sql = "SELECT * FROM `{$this->App->prefix()}supplier_street` WHERE  supplier_id={$suppId}";
        ...
        $this->template($mb.'/supplier_category');
    }
    ...
    function get_relate_cate_brand($cid = 0, $list = 20) {
        ...
        $sql = "SELECT distinct b.brand_id,b.brand_name,b.is_shop_hot,b.is_promote FROM `{$this->App->prefix()}brand` AS b LEFT JOIN `{$this->App->prefix()}goods` AS g ON b.brand_id = g.brand_id WHERE " . db_create_in($cids, 'g.cat_id') . " AND g.is_on_sale = '1' ORDER BY b.sort_order ASC,g.cat_id DESC LIMIT 50";
    ...
    //获商品子自分类cat_id
    function get_goods_sub_cat_ids($cid = 0,$suppId = 0) {
        //if(!($cid>=0)) return false;
        $rts = $this->get_goods_cate_tree($cid,$suppId);
        if ($cid > 0) {
            $cids[] = $cid;
        }
        if (!empty($rts)) {
    ...
    //获取商品分类
    function get_goods_cate_tree($cid = 0,$suppId = 0) {
        $three_arr = array();
        $sql = 'SELECT count(cat_id) FROM `' . $this->App->prefix() . "supplier_category` WHERE supplier_id = '$suppId' and parent_id = '$cid' AND is_show = 1";
        if ($this->App->findvar($sql) || $cid == 0) {
            ...
            $sql .= " WHERE tb1.supplier_id = '$suppId' and  tb1.parent_id = '$cid' GROUP BY tb1.cat_id ORDER BY tb1.parent_id ASC,tb1.sort_order ASC, tb1.cat_id ASC";
            ...
            foreach ($res as $row) {
                ...
                if (isset($row['cat_id']) != NULL) {
                    $three_arr[$row['cat_id']]['cat_id'] = $this->get_goods_cate_tree($row['cat_id'],$suppId);
                }
            }
        }
        return $three_arr;
    ...
    function top10($cid = 0, $list = 10) {
        ...
        $sql = "SELECT goods_id, goods_name,shop_price, market_price, goods_thumb,goods_img,original_img,promote_start_date,promote_end_date,promote_price,pifa_price,is_promote,sale_count FROM `{$this->App->prefix()}goods` WHERE is_on_sale = '1' $w AND is_delete = '0' AND is_alone_sale='1' ORDER BY sale_count DESC,is_shop_hot DESC,goods_id DESC LIMIT $list";
        $rt = $this->App->find($sql);

        $rt_ = array();
        if (empty($rt) || count($rt) < $list) {
            $c = $list - count($rt);
            $sql = "SELECT goods_id, goods_name,goods_bianhao, sale_count, shop_price,pifa_price,market_price, goods_thumb, original_img, goods_img,promote_start_date,promote_end_date,promote_price,is_promote FROM `{$this->App->prefix()}goods` WHERE is_on_sale = '1' AND is_delete = '0' AND is_alone_sale='1' AND (is_shop_hot='1' OR is_shop_new='1' OR is_shop_best='1') ORDER BY sale_count DESC,goods_id DESC LIMIT $c";
    ...
    function recommend_goods($list = 3, $type = '', $data = 0) {
        ...
        if (file_exists($fn) && !$cache->GetClose()) {
            ...
            $wd = "";
            if (!empty($keyword)) {
                $wd = " (g.goods_name LIKE '%{$keyword}%' OR g.meta_keys LIKE '%{$keyword}%' OR b.brand_name LIKE '%{$keyword}%')";
            }
            if ($type == 'is_promote') {
                $t = mktime();
                ...
                if (in_array($type, array('is_shop_best', 'is_shop_new', 'is_shop_hot'))) {
            ...
            if (!empty($rt)) {
                foreach ($rt as $row) {
                    ...
                    if (1) {
                        ...
                        $rts[$k]['price'] > 0 ? $dd[] = $rts[$k]['price'] : "0.00";
                    } else {
                    ...
                    $rts[$k]['promote_price'] = $row['promote_price'];
                    $row['promote_price'] > 0 ? $dd[] = $row['promote_price'] : "0.00";
                    //抢购价格
                    ...
                    $row['qianggou_price'] > 0 ? $dd[] = $row['qianggou_price'] : "0.00";
    ...
    function ajax_getcategoodslist($data = array()) {
        ...
        if (!$page)
            $page = 1;
			  ...
			  $supplier_id = $wobj->supplier_id;
		if(!$supplier_id){
			$supplier_id = 0;
			}
				$this->set('suppId', $supplier_id);	
        ...
        if ($bid > 0) {
            ...
            $sql = "SELECT cat_name FROM `{$this->App->prefix()}supplier_category` WHERE cat_id='$cid' LIMIT 1";
            $rt['infoname'] = $this->App->findvar($sql);
        } else {
        ...
        //条件
        $comd = array('cid' => $cid, 'bid' => $bid, 'price' => $price, 'keyword' => $keyword, 'attr' => (!empty($attr) ? explode('|', $attr) : array()),'supplier_id' => $supplier_id);
		
        $orderby = " ORDER BY g.{$order_type} {$sort_type}";
        $start = ($page - 1) * $list;
        $tt = $this->App->__get_goods_count_category($comd); //获取商品的数量
        $rt['goods_count'] = $tt;
        $rt['categoodspage'] = Import::basic()->ajax_page($tt, $list, $page, 'get_categoods_page_lists', array($supplier_id,$cid, $bid, $price, $order_type, $sort_type, $list, $attr));
        $rt['categoodslist'] = $this->App->__get_categoods_list_category($comd, $orderby, $start, $list); //商品列表

        $this->set('rt', $rt);
        $con = $this->fetch('ajax_goods_connents', true);
        $result = array('error' => $err, 'message' => $con);
        die($json->encode($result));
    }

    //产品筛选
    function ajax_select_goods($data = array()) {
		
		 $supplier_id = isset($data['supplier_id']) && intval($data['supplier_id']) > 0 ? $data['supplier_id'] : 0;
		 
        $cid = isset($data['cid']) && intval($data['cid']) > 0 ? $data['cid'] : 0;
        $bid = isset($data['bid']) && intval($data['bid']) > 0 ? $data['bid'] : 0;
        $attr = isset($data['attr']) ? $data['attr'] : '';
        $price = isset($data['price']) ? $data['price'] : '';
        $keyword = isset($data['keyword']) ? $data['keyword'] : '';
        $page = 1;
        $list = isset($_COOKIE['DISPLAY_LIMIT']) && intval($_COOKIE['DISPLAY_LIMIT']) > 0 ? intval($_COOKIE['DISPLAY_LIMIT']) : 50;
        //display type
        if (!isset($_COOKIE['DISPLAY_TYPE']) || empty($_COOKIE['DISPLAY_TYPE']) || !in_array($_COOKIE['DISPLAY_TYPE'], array('list', 'gallery'))) {
            $rt['display'] = 'gallery';
        } else {
        ...
        $tt = $this->App->__get_goods_count_category($comd); //获取商品的数量
        $rt['goods_count'] = $tt;
        $rt['categoodspage'] = Import::basic()->ajax_page($tt, $list, $page, 'get_categoods_page_lists', array($supplier_id,$cid, $bid, $price, 'goods_id', 'DESC', $list, $attr));
        $rt['categoodslist'] = $this->App->__get_categoods_list_category($comd, $orderby, $start, $list); //商品列表
