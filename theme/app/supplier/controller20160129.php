...
class SupplierController extends Controller{
	...
	if(!($suppId>0)){
        ...
        //今日抢鲜
        $sql = "SELECT goods_id,goods_name,sort_desc ,goods_thumb,goods_img,pifa_price,shop_price,sale_count FROM `{$this->App->prefix()}goods` WHERE is_new='1' AND is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0  and  supplier_id={$suppId} ORDER BY sort_order ASC, goods_id DESC LIMIT 4";
        $new = $this->App->find($sql);
        $this->set('new', $new);
		
		 //推荐
        $sql = "SELECT goods_id,goods_name,sort_desc ,goods_thumb,goods_img,pifa_price,shop_price,promote_price,market_price,sale_count FROM `{$this->App->prefix()}goods` WHERE is_best='1' AND is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0  and supplier_id={$suppId} ORDER BY sort_order ASC, goods_id DESC LIMIT 6";
		//echo $sql;
        $tj = $this->App->find($sql);
        $this->set('tj', $tj);
		 //热销
        $sql = "SELECT goods_id,goods_name,sort_desc ,goods_thumb,goods_img,pifa_price,shop_price,promote_price,market_price,sale_count FROM `{$this->App->prefix()}goods` WHERE is_hot='1' AND is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0  and supplier_id={$suppId} ORDER BY sort_order ASC, goods_id DESC LIMIT 6";
		//echo $sql;
