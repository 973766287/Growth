    ...
    function ajax_order_op_user($id = 0, $op = "") {
        ...
        if ($op == "cancel_order")
            ...
            $this->App->update('goods_order_info', array('shipping_status' => '5'), 'order_id', $id);
			   ...
			   $this->App->insert('user_money_change', array('thism' => date('Y-m', mktime()), 'thismonth' => date('Y-m-d', mktime()), 'money' => $val, 'changedesc' => '商品欠款返商户账户', 'time' => mktime(), 'uid' => $uid));
            $sql = "SELECT SUM(money) FROM `{$this->App->prefix()}user_money_change` WHERE uid='$uid'";
...
function apply_index() {
        ...
        $this->title('入驻申请首页 - ' . $GLOBALS['LANG']['site_name']);
        if (!defined(NAVNAME))
            define('NAVNAME', $bm['title']);
        $this->set('rt', $rt);
        $this->template('apply_index');
    }
...
function apply_one() {
      	$uid = $this->Session->read('User.uid');
		   ...
		   $sql = "SELECT * FROM `{$this->App->prefix()}supplier` WHERE user_id={$uid}";
		...
		if($info){
			...
			if($info['applynum'] == 1){
					 ...
					 $this->jump(SITE_URL . 'user.php?act=apply_two2');
            exit;
					
				
			}else if($info['applynum'] == 2){
				
			  $this->jump(SITE_URL . 'user.php?act=apply_three');
            exit;
			
			}else if($info['applynum'] == 3){
				
			  $this->jump(SITE_URL . 'user.php?act=apply_four');
            exit;
			
			}else if($info['applynum'] == 0){
				 $this->jump(SITE_URL . 'user.php?act=apply_two');
				   exit;
				}
		}
		else 
        ...
        $this->title('入驻须知 - ' . $GLOBALS['LANG']['site_name']);
     
        $this->template('apply_one');
		}
    }


 function ajax_apply($data) {
        $json = Import::json();
		$uid = $this->Session->read('User.uid');
		
		if($data['agreement'] == 'false'){
			
            $result = array('error' => 1, 'message' => '请点击同意入驻协议！');
            die($json->encode($result));
      
		}
		else{
			
			  $sql = "insert into `{$this->App->prefix()}supplier` (user_id) values ('{$uid}') ";
...
function apply_two($shownum="") {
     ...
     $uid = $this->Session->read('User.uid');
	  ...
	  //PC端首页banner广告位 
        ...
        $rt['ad37'] = $this->App->find($sql);
		 $this->set('rt', $rt);
		   $this->title('公司信息认证 - ' . $GLOBALS['LANG']['site_name']);
       ...
       $sql = "SELECT * FROM `{$this->App->prefix()}supplier` WHERE user_id = {$uid}";
		...
		$sql="SELECT s.*,sr.rank_name FROM  `{$this->App->prefix()}supplier`  as s left join `{$this->App->prefix()}supplier_rank` as sr ON s.rank_id=sr.rank_id
        WHERE s.supplier_id= {$rt['supplier_id']}";
        $suppinfo = $this->App->findrow($sql);
        $this->set('supplier', $suppinfo);
		
		
		  //地区
		   $sql = "SELECT region_id,region_name FROM `{$this->App->prefix()}region` WHERE region_type=1 and parent_id=1 ORDER BY region_id ASC";
		   $resslist = $this->App->find($sql);
	       $this->set('resslist', $resslist);
		   
		   $sql = "SELECT region_id,region_name FROM `{$this->App->prefix()}region` WHERE region_type=2 and parent_id={$suppinfo['province']} ORDER BY region_id ASC";
		   $dbress = $this->App->find($sql);
	       $this->set('city', $dbress);
		   
		    $sql = "SELECT region_id,region_name FROM `{$this->App->prefix()}region` WHERE region_type=3 and parent_id={$suppinfo['city']} ORDER BY region_id ASC";
		   $dbress = $this->App->find($sql);
	       $this->set('district', $dbress);
			 //  $rt['province'] = $this->get_regions(1);  //获取省列表
	
		if($rt){
		if( !empty($shownum)){
       $this->template('apply_two');
	    }else{
		 if($rt['applynum'] == 3){
			 $this->jump(SITE_URL . 'user.php?act=apply_four');
				   exit;
				}else if($rt['applynum'] == 2){
				
			  $this->jump(SITE_URL . 'user.php?act=apply_three');
            exit;
			
			}else if($rt['applynum'] == 1){
        ...
        $this->template('apply_two');
		 ...
		 $this->jump(SITE_URL . 'user.php?act=apply_one');
				   exit;
		}
    }
	
	
		function apply_two2($shownum="") {
     ...
     $uid = $this->Session->read('User.uid');
	  ...
	  //PC端首页banner广告位 
		...
		if($rt){
	if( !empty($shownum)){
        $this->template('apply_two2');
	}else{
		if($rt['applynum'] == 2){
			 $this->jump(SITE_URL . 'user.php?act=apply_three');
				   exit;
				}else if($rt['applynum'] == 3){
					 $this->jump(SITE_URL . 'user.php?act=apply_four');
				   exit;
     ...
     $uid = $this->Session->read('User.uid');
      ...
      $sql = "SELECT * FROM `{$this->App->prefix()}supplier` WHERE user_id = {$uid}";
        ...
        $this->set('supplier', $suppinfo);
		 ...
		 $sql = "SELECT * FROM `{$this->App->prefix()}street_category` where is_show='1'";
       $supplier_type = $this->App->find($sql);
		
		$this->set('supplier_type', $supplier_type);

		if($rt){
			if( !empty($shownum)){
        $this->template('apply_three');
	           }else{
		if($rt['applynum'] == 3){
			 $this->jump(SITE_URL . 'user.php?act=apply_four');
				   exit;
				}else if($rt['applynum'] == 0){
				 $this->jump(SITE_URL . 'user.php?act=apply_two');
				   exit;
				}else if($rt['applynum'] == 1){
				 $this->jump(SITE_URL . 'user.php?act=apply_two2');
...
function apply_four() {
	...
	$suppinfo['type_name'] = $info['str_name'];
        $this->set('supplier', $suppinfo);
	...
	//zzzzzzzzzzzzzzzzzzzzzzzzzzzzz个人信息认证
	  function ajax_apply_two1($data = array()) {
		  $uid = $this->Session->read('User.uid');
        $json = Import::json();
        ...
        //以下字段对应评论的表单页面 一定要一致
           ...
           $save['company_name'] = isset($fromAttr->company_name) ? trim(addslashes(htmlspecialchars($fromAttr->company_name))) : ''; 
		$save['country'] = isset($fromAttr->country) ? intval($fromAttr->country) : 1; 
		$save['province'] = isset($fromAttr->province) ? intval($fromAttr->province) : 1; 
		$save['city'] = isset($fromAttr->city) ? intval($fromAttr->city) : 1; 
		$save['district'] = isset($fromAttr->district) ? intval($fromAttr->district) : 1; 
		$save['address'] = isset($fromAttr->address) ? trim(addslashes(htmlspecialchars($fromAttr->address))) : '';
			
			$save['contacts_name'] = isset($fromAttr->contacts_name) ? trim(addslashes(htmlspecialchars($fromAttr->contacts_name))) : '';
			$save['contacts_phone'] = isset($fromAttr->contacts_phone) ? trim(addslashes(htmlspecialchars($fromAttr->contacts_phone))) : '';
			$save['email'] = isset($fromAttr->email) ? trim($fromAttr->email) : '';

			$save['id_card_no'] = isset($fromAttr->id_card_no) ? trim(addslashes(htmlspecialchars($fromAttr->id_card_no))) : '';

			$save['bank_account_name'] = isset($fromAttr->bank_account_name) ? trim(addslashes(htmlspecialchars($fromAttr->bank_account_name))) : '';
			$save['bank_account_number'] = isset($fromAttr->bank_account_number) ? trim(addslashes(htmlspecialchars($fromAttr->bank_account_number))) : '';
			$save['bank_name'] = isset($fromAttr->bank_name) ? trim(addslashes(htmlspecialchars($fromAttr->bank_name))) : '';
			$save['bank_code'] = isset($fromAttr->bank_code) ? trim(addslashes(htmlspecialchars($fromAttr->bank_code))) : '';
            $save['handheld_idcard'] = isset($fromAttr->handheld_idcard) ? trim(addslashes(htmlspecialchars($fromAttr->handheld_idcard))) : '';
            $save['idcard_front'] = isset($fromAttr->idcard_front) ? trim(addslashes(htmlspecialchars($fromAttr->idcard_front))) : '';
            $save['idcard_reverse'] = isset($fromAttr->idcard_reverse) ? trim(addslashes(htmlspecialchars($fromAttr->idcard_reverse))) : '';
			
			$save['applynum'] = isset($fromAttr->applynum) ? intval($fromAttr->applynum) : 0;
			$this->App->update('supplier', $save, 'user_id', $uid);
     
	//  $sql = "update `{$this->App->prefix()}supplier`  set company_name = '{$save['company_name']}'   where user_id={$uid} ";
//                 
//                    $this->App->query($sql);
	  ...
	  function ajax_apply_two2($data = array()) {
		  $uid = $this->Session->read('User.uid');
        $json = Import::json();
		...
		$save['district'] = isset($fromAttr->district) ? intval($fromAttr->district) : 1; 
		$save['address'] = isset($fromAttr->address) ? trim(addslashes(htmlspecialchars($fromAttr->address))) : '';
		$save['tel'] = isset($fromAttr->tel) ? trim(addslashes(htmlspecialchars($fromAttr->tel))) : '';
		$save['guimo'] = isset($fromAttr->guimo) ? trim(addslashes(htmlspecialchars($fromAttr->guimo))) : '';
		$save['email'] = isset($fromAttr->email) ? trim($fromAttr->email) : '';
		$save['company_type'] = isset($fromAttr->company_type) ? trim($fromAttr->company_type) : '';
		$save['contacts_name'] = isset($fromAttr->contacts_name) ? trim(addslashes(htmlspecialchars($fromAttr->contacts_name))) : '';
	    $save['contacts_phone'] = isset($fromAttr->contacts_phone) ? trim(addslashes(htmlspecialchars($fromAttr->contacts_phone))) : '';
	    $save['business_licence_number'] = isset($fromAttr->business_licence_number) ? trim(addslashes(htmlspecialchars($fromAttr->business_licence_number))) : '';
	    $save['business_sphere'] = isset($fromAttr->business_sphere) ? trim(addslashes(htmlspecialchars($fromAttr->business_sphere))) : '';
	    $save['organization_code'] = isset($fromAttr->organization_code) ? trim(addslashes(htmlspecialchars($fromAttr->organization_code))) : '';
  $save['zhizhao'] = isset($fromAttr->zhizhao) ? trim(addslashes(htmlspecialchars($fromAttr->zhizhao))) : '';
  $save['organization_code_electronic'] = 
  isset($fromAttr->organization_code_electronic) ? trim(addslashes(htmlspecialchars($fromAttr->organization_code_electronic))) : '';
  $save['general_taxpayer'] = isset($fromAttr->general_taxpayer) ? trim(addslashes(htmlspecialchars($fromAttr->general_taxpayer))) : '';
	 ...
	 function ajax_apply_two2_store($data = array()) {
		  $uid = $this->Session->read('User.uid');
        $json = Import::json();
        ...
        $save['bank_account_name'] = 
		isset($fromAttr->bank_account_name) ? trim(addslashes(htmlspecialchars($fromAttr->bank_account_name))) : '';
	    $save['bank_account_number'] = 
		isset($fromAttr->bank_account_number) ? trim(addslashes(htmlspecialchars($fromAttr->bank_account_number))) : '';
	    $save['bank_name'] = 
		isset($fromAttr->bank_name) ? trim(addslashes(htmlspecialchars($fromAttr->bank_name))) : '';
	    $save['bank_code'] = 
		isset($fromAttr->bank_code) ? trim(addslashes(htmlspecialchars($fromAttr->bank_code))) : '';
	    $save['settlement_bank_account_name'] = 
		isset($fromAttr->settlement_bank_account_name) ? trim(addslashes(htmlspecialchars($fromAttr->settlement_bank_account_name))) : '';
	    $save['settlement_bank_account_number'] = 
		isset($fromAttr->settlement_bank_account_number) ? trim(addslashes(htmlspecialchars($fromAttr->settlement_bank_account_number))) : '';
	    $save['settlement_bank_name'] = 
		isset($fromAttr->settlement_bank_name) ? trim(addslashes(htmlspecialchars($fromAttr->settlement_bank_name))) : '';
	    $save['settlement_bank_code'] = 
		isset($fromAttr->settlement_bank_code) ? trim(addslashes(htmlspecialchars($fromAttr->settlement_bank_code))) : '';
	    $save['tax_registration_certificate'] = 
		isset($fromAttr->tax_registration_certificate) ? trim(addslashes(htmlspecialchars($fromAttr->tax_registration_certificate))) : '';
	    $save['taxpayer_id'] = 
		isset($fromAttr->taxpayer_id) ? trim(addslashes(htmlspecialchars($fromAttr->taxpayer_id))) : '';
		
		$save['bank_licence_electronic'] = 
		isset($fromAttr->bank_licence_electronic) ? trim(addslashes(htmlspecialchars($fromAttr->bank_licence_electronic))) : '';
		$save['tax_registration_certificate_electronic'] = 
isset($fromAttr->tax_registration_certificate_electronic) ? trim(addslashes(htmlspecialchars($fromAttr->tax_registration_certificate_electronic))) : '';
		$save['applynum'] = isset($fromAttr->applynum) ? intval($fromAttr->applynum) : 0;
	 ...
	 function ajax_apply_three($data = array()) {
		  $uid = $this->Session->read('User.uid');
        $json = Import::json();
	    ...
	    $save['supplier_name'] = isset($fromAttr->supplier_name) ? trim(addslashes(htmlspecialchars($fromAttr->supplier_name))) : ''; 
	//	$save['rank_id'] = isset($fromAttr->rank_id) ? intval($fromAttr->rank_id) : 0; 
		$save['type_id'] = isset($fromAttr->type_id) ? intval($fromAttr->type_id) : 0; 
		
		$save['applynum'] = isset($fromAttr->applynum) ? intval($fromAttr->applynum) : 0;
		
		
		
			  $sql = "SELECT user_rank FROM `{$this->App->prefix()}user` WHERE user_id = {$uid}";
        $rt = $this->App->findrow($sql);
		
		$save['rank_id'] = $rt['user_rank'];
