...
class IndexController extends Controller {
    ...
    function index() {
        ...
        //今日抢鲜
        $sql = "SELECT goods_id,goods_name,goods_brief ,goods_thumb,goods_img,pifa_price,shop_price,sale_count FROM `{$this->App->prefix()}goods` WHERE is_best='1' AND is_on_sale='1' AND is_alone_sale='1' AND is_jifen='0' AND is_delete = '0' and is_prize=0  and is_best ='1' ORDER BY sort_order ASC, goods_id DESC LIMIT 4";
        ...
        $sql = "SELECT   goods_id,goods_name,sort_desc ,goods_thumb,goods_img,pifa_price,shop_price,sale_count  from `{$this->App->prefix()}goods`   WHERE cat_id in($ids) AND is_on_sale='1' AND is_alone_sale='1' and is_prize=0 AND is_delete = '0'  AND is_jifen='0'   ORDER BY sort_order ASC, goods_id DESC  LIMIT 10";
        ...
        $ids = implode(",", $idarr);
        $sql = "SELECT   goods_id,goods_name,goods_brief ,goods_thumb,goods_img,pifa_price,shop_price,sale_count  from `{$this->App->prefix()}goods`   WHERE cat_id in($ids) AND is_on_sale='1' AND is_alone_sale='1' and is_prize=0 AND is_delete = '0'  AND is_jifen='0'   LIMIT 10";
