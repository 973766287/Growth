...
class CatalogController extends Controller {
    ...
    function index($cid = 0, $page = 0, $rs = array()) {
        ...
        if (is_null($rt)) {
            ...
            if (isset($_GET['desc'])) {
                ...
                $orderby = ' ORDER BY g.sort_order ASC ,g.supplier_id asc, g.goods_id DESC ';
    ...
    function get_goods_cate_tree($cid = 0) {
        ...
        if ($this->App->findvar($sql) || $cid == 0) {
            $sql = 'SELECT tb1.cat_name,tb1.cat_id,tb1.parent_id,tb1.is_show,tb1.cat_title,tb1.cat_url FROM `' . $this->App->prefix() . "goods_cate` AS tb1";
            ...
            foreach ($res as $row) {
                ...
                $three_arr[$row['cat_id']]['url'] = empty($row['cat_url']) ? 'catalog' : $row['cat_url'];
    ...
    function ajax_select_goods($data = array()) {
        ...
        $tt = $this->App->__get_goods_count_category($comd); //获取商品的数量
        $rt['goods_count'] = $tt;
        $rt['categoodspage'] = Import::basic()->ajax_page($tt, $list, $page, 'get_categoods_page_list', array($cid, $bid, $price, 'goods_id', 'DESC', $list, $attr));
        $rt['categoodslist'] = $this->App->__get_categoods_list_category($comd, $orderby, $start, $list); //商品列表
    ...
    //end function 
	  ...
	  function street($cid = 0, $page = 0, $rs = array()){
		  
		    $sql = "SELECT * FROM `{$this->App->prefix()}street_category` WHERE  is_show=1  order by is_groom desc,sort_order asc";
			  
            $rt['street'] = $this->App->find($sql);

	  
	   $id = (isset($_GET['id']) && intval($_GET['id']) > 0) ? intval($_GET['id']) : $id;
        ...
        $w = "";
		if(isset($id) && !empty($id)){ $w = "supplier_type = $id and ";}
		
            $start = ($page - 1) * $list;
			
			  $sql = "SELECT count(*) FROM `{$this->App->prefix()}supplier_street` WHERE $w status=1 and is_show=1  order by is_groom desc,sort_order asc";
			...
			$sql = "SELECT * FROM `{$this->App->prefix()}supplier_street` WHERE $w status=1 and is_show=1  order by is_groom desc,sort_order asc limit $start, $list";
			
			
            $rt['categoodslist'] = $this->App->find($sql); //店铺列表
			
			

	   $this->set('rt', $rt);
	    $this->set('page', $page);
	  $this->title('店铺街 - ' . $GLOBALS['LANG']['site_name']);
